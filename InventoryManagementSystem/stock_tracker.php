...
// DEBUG: Check selected date
echo "<!-- DEBUG: Selected date: " . $selected_date . " -->\n";

// Check kung may movements sa specific date na ito
$check_sql = "SELECT COUNT(*) as total FROM stock_movements WHERE DATE(created_at) = ?";
$check_stmt = $conn->prepare($check_sql);
$check_stmt->bind_param("s", $selected_date);
$check_stmt->execute();
$check_result = $check_stmt->get_result();
$check_count = $check_result->fetch_assoc();
echo "<!-- DEBUG: Actual movements on this date: " . $check_count['total'] . " -->\n";

// Get stock movements for the selected date WITH COMPLETE DETAILS - IMPROVED VERSION
...
// DEBUG: Check daily movements count
echo "<!-- DEBUG: Daily movements count: " . ($daily_movements ? $daily_movements->num_rows : 0) . " -->\n";

// DEBUG: Show first record if exists
if ($daily_movements && $daily_movements->num_rows > 0) {
    $daily_movements->data_seek(0);
    $first = $daily_movements->fetch_assoc();
    echo "<!-- DEBUG: First record - ";
    echo "Item No: " . (isset($first['item_no']) ? $first['item_no'] : 'N/A') . ", ";
    echo "Description: " . (isset($first['description']) ? $first['description'] : 'N/A') . ", ";
    echo "Unit: " . (isset($first['unit']) ? $first['unit'] : 'N/A') . ", ";
    echo "Created: " . (isset($first['created_at']) ? $first['created_at'] : 'N/A') . " -->\n";
    $daily_movements->data_seek(0); // Reset pointer
}
...
<div class="stock-tracker-container">
    ...
    <!-- Simple notification na nasa Stock Movements na ang purchase -->
    <?php if(isset($_GET['purchased']) && $_GET['purchased'] == 'success'): ?>
    <div class="alert alert-success" style="margin-bottom: 20px; padding: 15px; background: rgba(117, 230, 218, 0.2); border-left: 4px solid #75e6da; border-radius: 8px;">
        <i class="fas fa-check-circle" style="color: #75e6da; margin-right: 10px;"></i>
        <strong>Purchase successful!</strong> The item has been added to Stock Movements below.
    </div>
    <?php endif; ?>
    ...
    <div id="dailyTab" class="tab-content active">
        <div class="tracker-container">
            ...
            <table class="tracker-table">
                ...
                <tbody>
                  <!-- Sa loob ng tracker table, palitan ang row display -->
<?php if ($daily_movements && $daily_movements->num_rows > 0): ?>
    ...
    <tr>
        ...
        <td>
            ...
            <?php if (strpos($movement['reference'], 'PURCHASE') !== false): ?>
                <span style="display: block; font-size: 10px; color: #75e6da; margin-top: 3px;">
...
<div id="referencesModal" class="modal">
    <div class="modal-content modal-sm">
        <div class="modal-header">
            <h2><i class="fas fa-list"></i> All References</h2>
            <button class="close-btn" onclick="closeReferencesModal()">&times;</button>
        </div>
        <div class="modal-body" id="referencesModalContent">
            <!-- Content will be loaded dynamically -->
        </div>
        <div class="modal-footer">
            <button class="btn btn-secondary" onclick="closeReferencesModal()">
...
document.addEventListener('DOMContentLoaded', function() {
    // Check if there's a movement to highlight
    const urlParams = new URLSearchParams(window.location.search);
    const highlightMovement = urlParams.get('highlight');
    
    if (highlightMovement) {
        // Find and highlight the row with this movement
        setTimeout(() => {
            highlightNewPurchase(highlightMovement);
        }, 500);
    }
});

// Function to highlight new purchase in stock tracker
function highlightNewPurchase(movementId) {
    // Look for the reference tag containing this movement
    const referenceTags = document.querySelectorAll('.reference-tag');
    
    referenceTags.forEach(tag => {
        if (tag.textContent.includes('PURCHASE')) {
            // Add highlight effect
            tag.style.animation = 'pulse 2s 3';
            tag.style.boxShadow = '0 0 20px #75e6da';
            
            // Scroll to the row
            const row = tag.closest('tr');
            if (row) {
                row.style.background = 'rgba(117, 230, 218, 0.2)';
...
// Add this CSS to your stock_tracker.php style section
const style = document.createElement('style');
style.textContent = `
    @keyframes pulse {
        0% { transform: scale(1); }
        50% { transform: scale(1.05); box-shadow: 0 0 30px #75e6da; }
...
// Show all references
function showAllReferences(element) {
    const references = JSON.parse(element.getAttribute('data-references'));
    const modalContent = document.getElementById('referencesModalContent');
    
    let html = '<div style="padding: 10px;">';
    html += '<h3 style="margin-bottom: 15px; color: var(--text-primary);">All References for this Product:</h3>';
    html += '<div style="display: flex; flex-direction: column; gap: 8px;">';
    
    references.forEach(ref => {
        html += '<div style="padding: 8px; background: var(--bg-secondary); border-radius: 5px;">' + ref + '</div>';
    });
...
function exportDailyData() {
    ...
    let csv = 'Date,Item No,Description,Unit,Quantity,Reference\n';
    
    rows.forEach(row => {
        if (row.style.display !== 'none' && row.cells.length > 1) {
            const cells = row.querySelectorAll('td');
            const rowDate = document.getElementById('currentDateDisplay').textContent;
            ...
            csv += `"${rowDate}","${itemNo}","${description}","${unit}","${quantity}","${reference}"\n`;
        }
    });
...
window.onclick = function(event) {
    const modals = ['stockInModal', 'stockOutModal', 'referencesModal'];
