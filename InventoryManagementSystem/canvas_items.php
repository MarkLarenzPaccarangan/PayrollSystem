...
if (isset($_POST['add_company'])) {
    ...
    if (!empty($name)) {
        $stmt = $conn->prepare("INSERT INTO companies (name, description, contact_person, contact_number) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $name, $description, $contact_person, $contact_number);
...
if (isset($_POST['edit_company'])) {
    ...
    $stmt = $conn->prepare("UPDATE companies SET name = ?, description = ?, contact_person = ?, contact_number = ? WHERE id = ?");
    $stmt->bind_param("ssssi", $name, $description, $contact_person, $contact_number, $company_id);
    
    if ($stmt->execute()) {
        $_SESSION['message'] = "Company updated successfully!";
        $_SESSION['msg_type'] = "success";
    } else {
        $_SESSION['message'] = "Error updating company: " . $conn->error;
        $_SESSION['msg_type'] = "danger";
    }
    $stmt->close();
    header("Location: canvas_items.php");
            ...
            foreach ($_POST['companies'] as $index => $data) {
                if (!empty($data['company_name']) && (!empty($data['quantity']) || !empty($data['price']))) {
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        // Update contact person and number if provided
                        if (!empty($contact_person) || !empty($contact_number)) {
                            $update_contact = $conn->prepare("UPDATE companies SET contact_person = ?, contact_number = ? WHERE id = ?");
                            $update_contact->bind_param("ssi", $contact_person, $contact_number, $company_id);
                            $update_contact->execute();
                            $update_contact->close();
                        }
                    } else {
                        // Create new company with contact person and number
                        $insert_company = $conn->prepare("INSERT INTO companies (name, contact_person, contact_number) VALUES (?, ?, ?)");
                        $insert_company->bind_param("sss", $company_name, $contact_person, $contact_number);
...
// Get all companies (including contact_person and contact_number)
...
.company-fields {
    display: grid;
    grid-template-columns: 1.5fr 1fr 1fr 1fr 1fr 1fr auto; /* Added columns for contact number and total display */
    gap: 10px;
    align-items: center;
}
            ...
            <?php if ($items && $items->num_rows > 0): ?>
                            ...
                            if (!empty($part)) {
                                ...
                                if (isset($companies_array[$company_id])) {
                                    $item_prices[] = [
                                        ...
                                        'contact_number' => $companies_array[$company_id]['contact_number'] ?? '',
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            <?php if (!empty($price_data['contact_number'])): ?>
                                <span class="contact-number">
                                    <i class="fas fa-phone"></i> <?php echo htmlspecialchars($price_data['contact_number']); ?>
                                </span>
                            <?php else: ?>
                                <span class="text-muted">—</span>
                            <?php endif; ?>
                ...
                <div id="dynamicCompanyFields">
                    ...
                    <div class="company-row" id="company-row-0">
                        ...
                        <div class="company-fields">
                            ...
                            <input type="text" name="companies[0][contact_person]" placeholder="Contact Person" id="contact_0">
                            <input type="text" name="companies[0][contact_number]" placeholder="Contact Number" id="contact_number_0">
...
function addCompanyRow() {
    ...
    newRow.innerHTML = `
        ...
        <div class="company-fields">
            ...
            <input type="text" name="companies[${companyRowCount}][contact_person]" placeholder="Contact Person" id="contact_${companyRowCount}">
            <input type="text" name="companies[${companyRowCount}][contact_number]" placeholder="Contact Number" id="contact_number_${companyRowCount}">
        ...
        .then(data => {
            ...
            data.forEach(item => {
                item.prices.forEach(price => {
                    ...
                    const contactNumber = price.contact_number || '—';
            ...
            document.getElementById('comparisonTableBody').innerHTML = html || '<tr><td colspan="9" class="empty-state">No data available</td></tr>';
            ...
            if (data.prices && Object.keys(data.prices).length > 0) {
                    ...
                    html += `
                        <div class="company-row" id="edit-company-row-${editCompanyRowCount}">
                            ...
                            <div class="company-fields">
                                ...
                                <input type="text" name="companies[${editCompanyRowCount}][contact_person]" value="${contactPerson}" placeholder="Contact Person" id="edit_contact_${editCompanyRowCount}">
                                <input type="text" name="companies[${editCompanyRowCount}][contact_number]" value="${contactNumber}" placeholder="Contact Number" id="edit_contact_number_${editCompanyRowCount}">
                ...
                html = `
                    <div class="company-row" id="edit-company-row-0">
                        ...
                        <div class="company-fields">
                            <input type="text" name="companies[0][company_name]" placeholder="Company Name" required>
                            <input type="text" name="companies[0][contact_person]" placeholder="Contact Person" id="edit_contact_0">
                            <input type="text" name="companies[0][contact_number]" placeholder="Contact Number" id="edit_contact_number_0">
...
function addEditCompanyRow() {
    ...
    newRow.innerHTML = `
        ...
        <div class="company-fields">
            ...
            <input type="text" name="companies[${editCompanyRowCount}][contact_person]" placeholder="Contact Person" id="edit_contact_${editCompanyRowCount}">
            <input type="text" name="companies[${editCompanyRowCount}][contact_number]" placeholder="Contact Number" id="edit_contact_number_${editCompanyRowCount}">
