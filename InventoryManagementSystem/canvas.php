...
if (!empty($search_item_no)) {
    $query .= " AND ci.item_no LIKE '%$search_item_no%'";
}
...
// Handle POST request for adding new company price
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_company_price') {
    ...
    // Check if item exists in canvas_items
    $check_item = $conn->query("SELECT id FROM canvas_items WHERE item_no = '$item_no'");
    if ($check_item->num_rows == 0) {
        // Insert new canvas item
        $insert_item = $conn->query("INSERT INTO canvas_items (item_no, description) VALUES ('$item_no', '$description')");
...
<div class="item-search-section">
    ...
    <div class="price-sort-buttons">
        <button class="price-sort-btn <?php echo ($active_sort == 'price' && $sort_by == 'price_asc') ? 'active' : ''; ?>" onclick="applyPriceSort('price_asc')">
            <i class="fas fa-sort-amount-down"></i> Price Low to High
        </button>
        <button class="price-sort-btn <?php echo ($active_sort == 'price' && $sort_by == 'price_desc') ? 'active' : ''; ?>" onclick="applyPriceSort('price_desc')">
                        ...
                        <td>
                            <div style="display: flex; gap: 5px; justify-content: center;">
                                ...
                                <?php if($row['availability'] && $row['available_quantity'] > 0): ?>
                                    <button class="action-btn add-to-cart" onclick="openCartModal(this)" id="btn-<?php echo $row['price_id']; ?>" title="Add to Cart">
                ...
                <tr>
                    <td colspan="10" style="text-align: center; padding: 60px;">
                        <div class="empty-state">
                            ...
                            <button class="action-btn add-company" onclick="openCompanyFormModal()">
                                <i class="fas fa-building"></i> Add Company Price
                            </button>
                        </div>
                    </td>
                </tr>
    ...
    <div class="modal-content company-modal">
        ...
        <div class="modal-body">
            <form method="POST" action="canvas.php" id="companyForm">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label class="form-label">
                            <i class="fas fa-hashtag"></i> Item No *
                        </label>
                    ...
                    <div class="form-group">
                        <label class="form-label">
                            <i class="fas fa-cubes"></i> Available Qty *
                        </label>
                ...
                <!-- Form Actions -->
                <div class="modal-actions">
                    <button type="button" class="btn-cancel" onclick="closeCompanyFormModal()">
                        <i class="fas fa-times"></i> Cancel
                    </button>
                    <button type="submit" class="btn-save-company" id="saveCompanyBtn">
                        <i class="fas fa-save"></i> Save Company Price
                    </button>
                </div>
            </form>
        </div>
...
<div id="viewModal" class="modal">
    <div class="modal-content view-modal">
        ...
        <div class="modal-body">
            <div class="view-details-grid" id="viewDetails">
                <!-- Details will be populated by JavaScript -->
            </div>
            ...
            <form id="editForm">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="editItemNo" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="editQuantity" min="1" required onchange="calculateEditTotal()" onkeyup="calculateEditTotal()">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-row">
                            <span style="color: var(--text-primary); font-weight: 600;">₱</span>
                            <input type="number" class="form-control" id="editPrice" step="0.01" min="0" required onchange="calculateEditTotal()" onkeyup="calculateEditTotal()">
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-actions">
                    ...
                    <button type="submit" class="btn-save-edit" id="saveEditBtn">
    ...
    <div class="modal-content delete-modal">
        ...
        <div class="modal-body">
            ...
            <div class="delete-modal-actions">
              ...
              <button class="btn-delete-confirm" onclick="deleteNow()">
            ...
            <div class="comparison-header">
                ...
                <div class="comparison-actions">
                    <div class="comparison-sort">
                        <button class="comparison-sort-btn" onclick="sortComparison('price_asc')" id="sortPriceAscBtn">
                            <i class="fas fa-sort-amount-down"></i> Price Low-High
                        </button>
                        <button class="comparison-sort-btn" onclick="sortComparison('price_desc')" id="sortPriceDescBtn">
                            <i class="fas fa-sort-amount-up"></i> Price High-Low
                        </button>
                        <button class="comparison-sort-btn" onclick="sortComparison('company_asc')" id="sortCompanyAscBtn">
                            <i class="fas fa-sort-alpha-down"></i> Company A-Z
                        </button>
                        <button class="comparison-sort-btn" onclick="sortComparison('company_desc')" id="sortCompanyDescBtn">
                            <i class="fas fa-sort-alpha-up"></i> Company Z-A
                        </button>
                    </div>
...
<script>
    // New delete function - WORKING VERSION
function deleteNow() {
    console.log('deleteNow called');
    
    // Get the pending ID
    var id = null;
    
    if (typeof pendingDeleteId !== 'undefined') {
        id = pendingDeleteId;
    } else if (typeof window.pendingDeleteId !== 'undefined') {
        id = window.pendingDeleteId;
    }
...
// FILTER TABLE FUNCTION - CLIENT-SIDE LANG (WALANG PAGE RELOAD)
...
function applyPriceSort(sortValue) {
    ...
    if (searchTerm) {
        url.searchParams.set('search_item', searchTerm);
    }
...
// Add to cart function - GOES TO PURCHASE.PHP
function addToCart() {
    ...
    // Prepare data for database - PENDING status
    const purchaseData = {
...
// Edit Company Price - LAHAT NG FIELDS PWEDENG PALITAN
function editCompanyPrice(priceId) {
    console.log('Editing price ID:', priceId); // For debugging
    ...
    // Populate edit form - LAHAT NG FIELDS
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editForm) {
        editForm.addEventListener('submit', function(e) {
            ...
            const itemNo = document.getElementById('editItemNo').value.trim();
            ...
            // Validate
            if (!itemNo) {
                showNotification('Item No is required', 'error');
                return;
            }
...
// Delete Company Price - with enhanced custom modal
function deleteCompanyPrice(priceId, companyName, itemNo) {
    ...
    document.getElementById('deleteItemNo').textContent = itemNo;
...
function sortComparison(type) {
    ...
    document.getElementById('sortPriceAscBtn').classList.remove('active');
    document.getElementById('sortPriceDescBtn').classList.remove('active');
    document.getElementById('sortCompanyAscBtn').classList.remove('active');
    document.getElementById('sortCompanyDescBtn').classList.remove('active');
    
    switch(type) {
        case 'price_asc':
            items.sort((a, b) => a.price - b.price);
            document.getElementById('sortPriceAscBtn').classList.add('active');
            break;
        case 'price_desc':
            items.sort((a, b) => b.price - a.price);
            document.getElementById('sortPriceDescBtn').classList.add('active');
            break;
        case 'company_asc':
            items.sort((a, b) => a.company.localeCompare(b.company));
            document.getElementById('sortCompanyAscBtn').classList.add('active');
            break;
        case 'company_desc':
            items.sort((a, b) => b.company.localeCompare(a.company));
            document.getElementById('sortCompanyDescBtn').classList.add('active');
            break;
    }
