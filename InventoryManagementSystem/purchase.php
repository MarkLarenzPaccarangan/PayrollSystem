...
<div id="historyPurchaseModal" class="modal">
    <div class="modal-content modal-xl">
        <div class="modal-header" style="background: linear-gradient(135deg, #3498db, #2980b9);">
            ...
            <button class="close-btn" onclick="closeHistoryPurchaseModal()" style="background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.3); color: white; width: 32px; height: 32px; border-radius: 6px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 18px; transition: all 0.3s ease;">
...
<div id="orderNowModal" class="modal">
    <div class="modal-content modal-sm">
        <div class="modal-header" style="background: linear-gradient(135deg, #00b894, #75e6da);">
            <h2><i class="fas fa-shopping-cart"></i> Confirm Order</h2>
            <button class="close-btn" onclick="closeOrderNowModal()">&times;</button>
        </div>
        <div class="modal-body" id="orderNowContent">
            <!-- Content will be loaded dynamically -->
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" onclick="closeOrderNowModal()">
                <i class="fas fa-times"></i> Cancel
            </button>
            <button type="button" class="btn btn-primary" id="confirmOrderBtn" style="background: linear-gradient(135deg, #00b894, #75e6da);" onclick="processOrderNow()">
...
<script>
// ============ ORDER NOW WITH CONFIRMATION MODAL ============
let currentOrderId = null;
let currentOrderBtn = null;

function orderNowFromHistory(purchaseId) {
    currentOrderId = purchaseId;
    currentOrderBtn = event.currentTarget;
    
    // Get purchase details from the row
    const row = currentOrderBtn.closest('tr');
    ...
    const content = `
        <div style="padding: 10px;">
            <p style="margin-bottom: 20px; color: var(--text-primary);">Are you sure you want to place this order?</p>
            
            <div style="background: rgba(117, 230, 218, 0.1); padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                <table style="width: 100%; font-size: 14px;">
                    <tr>
                        <td style="padding: 5px 0; color: var(--text-secondary);">Item No:</td>
                        <td style="padding: 5px 0; font-weight: 600; color: var(--text-primary);">${itemNo}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 0; color: var(--text-secondary);">Description:</td>
                        <td style="padding: 5px 0; color: var(--text-primary);">${description}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 0; color: var(--text-secondary);">Company:</td>
                        <td style="padding: 5px 0; color: var(--text-primary);">${company}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 0; color: var(--text-secondary);">Quantity:</td>
                        <td style="padding: 5px 0; font-weight: 600; color: #75e6da;">${qty}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 0; color: var(--text-secondary);">Price/Unit:</td>
                        <td style="padding: 5px 0; color: #75e6da;">${price}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 0; color: var(--text-secondary);">Total:</td>
                        <td style="padding: 5px 0; font-weight: 700; color: #6c5ce7;">${total}</td>
                    </tr>
                </table>
            </div>
            
            <p style="color: #f39c12; font-size: 13px; display: flex; align-items: center; gap: 5px;">
                <i class="fas fa-info-circle"></i> This will mark the order as COMPLETED and add it to Stock Tracker.
            </p>
        </div>
    `;
    
    document.getElementById('orderNowContent').innerHTML = content;
    document.getElementById('orderNowModal').style.display = 'block';
...
// Search function - UPDATED to include category
            ...
            if (data.success) {
                ...
                history.forEach((item, index) => {
                    historyRows += `
                        <tr class="history-row">
                            ...
                            <td class="price-cell history-total">${item.formatted_total}</td>
                            <td class="history-status"><span class="status-badge ${item.status_class}">${item.status.charAt(0).toUpperCase() + item.status.slice(1)}</span></td>
                            <td class="history-date">${item.formatted_date}</td>
                        </tr>
                    `;
                });
                ...
                const content = `
                    <div style="padding: 10px;">
                        ...
                        <!-- SEARCH BAR PARA SA TRANSACTION HISTORY -->
                        <div class="summary-bar" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                            <div style="display: flex; align-items: center; gap: 15px;">
                                <span style="font-size: 14px; color: var(--text-secondary);">Showing:</span>
                                <span style="font-size: 18px; font-weight: 600; color: #3498db;">${data.total_records} Completed Transactions</span>
                            </div>
                            ...
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <div class="search-wrapper" style="position: relative; width: 300px;">
                                    ...
                                    <input type="text" 
                                           ...
                                           style="padding: 10px 10px 10px 35px; background: var(--bg-secondary); border: 2px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; width: 100%;">
                                </div>
                                <span class="history-badge" id="historySearchCount">${data.total_records} items</span>
                            </div>
                        </div>
                        
                        <div style="overflow-x: auto; margin-top: 15px;">
                            <table class="products-table" style="min-width: 1400px;" id="historyTable">
                                <thead>
                                    <tr>
                        ...
                        <!-- No Results Message for History -->
                        <div id="historyNoResults" class="empty-state" style="padding: 40px; display: none;">
                            <i class="fas fa-search"></i>
                            <h3>No Matching Transactions</h3>
                            <p>No transactions match your search criteria. Try different keywords.</p>
                        </div>
...
function searchHistory() {
    ...
    rows.forEach(row => {
        ...
        const price = row.querySelector('.history-price')?.textContent.toLowerCase() || '';
        const total = row.querySelector('.history-total')?.textContent.toLowerCase() || '';
    ...
    if (searchCount) {
        if (searchTerm === '') {
            searchCount.textContent = `${rows.length} items`;
        } else {
            searchCount.textContent = `${visibleCount} of ${rows.length} items`;
        }
    }
...
document.addEventListener('keyup', function(e) {
    if (e.target && e.target.id === 'historySearch') {
...
window.onclick = function(event) {
    const modals = ['viewPurchaseModal', 'editPurchaseModal', 'deletePurchaseModal', 'historyPurchaseModal', 'orderNowModal'];
    modals.forEach(modalId => {
        ...
        if (event.target == modal) {
            ...
            } else if (modalId === 'deletePurchaseModal') {
                closeDeletePurchaseModal();
            } else if (modalId === 'historyPurchaseModal') {
                closeHistoryPurchaseModal();
            } else if (modalId === 'orderNowModal') {
                closeOrderNowModal();
            }
        }
    });
